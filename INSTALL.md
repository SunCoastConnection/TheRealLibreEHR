...
NOTE: Most recent documentation can be found on the online documentation at [Libre](http://LibreEHR.org/).
...
Alternatively you can download the source code directly from the repository located at [Libreehr](https://github.com/LibreIO/Libreehr) using

```
git clone https://github.com/LibreIO/LibreEHR Libreehr
```

##  Setup

To run LibreEHR, MariaDB (prefered) or MySQL, and Apache or another PHP-capable webserver must be configured.
...
To ensure proper functioning of LibreEHR you must make sure that settings in the `php.ini` file include:
```
...
Once the system has been configured properly, you may login.  Connect to the webserver where the files are stored with your web browser.  Login to the system using the username that you picked (default is `admin`), and the password.  From there, select the `Administration` option, and customize the system to your needs.  Add users and groups as is needed. For information on using LibreEHR, consult the User Documentation located in the `Documentation` folder, the documentation at [Libre](http://LibreEHR.org/).

Reading `Libreehr/sites/default/config.php` is a good idea.

To create custom encounter forms online documentation at [Libre](http://LibreEHR.org/). Many forms exist in `interface/forms` and may be used as examples.
...
phpGACL access controls are installed and configured automatically during LibreEHR setup.  It can be administered within LibreEHR in the admin->acl menu.  This is very powerful access control software.

Learn more about phpGACL [here](http://phpgacl.sourceforge.net/), recommend reading the phpGACL manual, the `/Libreehr/Documentation/README.phpgacl.md` file, and the online documentation at [Libre](http://LibreEHR.org/) . Also recommend reading the comments in `/Libreehr/library/acl.inc`.
 ...
 **Note:**
 ...
 Clone the LibreEHR [repository](https://github.com/LibreIO/LibreEHR) into your console eg. [GitBash](https://git-for-windows.github.io/), [Cmder](http://cmder.net/)
...
## Mac Setup With MAMP
Apache can be used on Mac to install Libre. However, if you wish to use MAMP, it is also an option.

#### Setup

#### Step 1
Download [MAMP](https://mamp.info/en/downloads/) version 5.1.
...
#### Step 3
Now navigate through go to the Mac menubar and navigate through MAMP > Preferences > Ports to change your Apache Port to 80 and MySQL port to 3306. This can be done by pressing 'Set Web and MySQL ports to 80 & 3306'.
...
#### Step 4
you should make sure that you have got the right PHP version. (7.2.x Recommended but 7.1.x works fine)

 ![MAMPPHP](./Documentation/1_Installing/images/MAMP_installation/PHP1.png)

#### Step 5
To make sure the LibreEHR functions properly, you will need to make some changes to your php.ini file. Different versions of php have different php.ini files.
...
Make the following changes in your php.ini file to ensure that LibreEHR functions properly:
...
Go to the LibreEHR Setup page in your browser: `localhost/Libreehr/setup.php` (If there is an Site can't be reached error, sure that you are on the right port, this can be found in the preference of your MAMP. Also ensure that your MAMP server has successfully started, this can be seen on the control panel)
...
**Note:** If you do change your password, or even if you don't, write down your username and password. You will need it for future uses.
 ...
 If this comes up, congratulations! You have successfully installed Libre EHR.
