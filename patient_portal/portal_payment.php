...
<script type="text/javascript" src="<?php echo $GLOBALS['standard_js_path']; ?>/jquery-creditcardvalidator-1-1-0/jquery.creditCardValidator.js"></script>
 ...
 for (var i = 0; i < f.elements.length; ++i) {
  ...
  if (ename.indexOf('form_upay[') == 0 || ename.indexOf('form_bpay[') == 0) {
   if (elem.value.length > 0){
     ...
     if(flag){
        $('#payfrm')[0].reset();
        alert("<?php echo xla('Negative payments not accepted'); ?>");
    }
 return true;
}
function coloring()
...
//CO-PAY
  ...
  {
    alert("<?php echo xla('Invalid Total!') ?>");
    return false;
  }
 if(ok==-1)
  {
     //return true;
     if(confirm("<?php echo xla('Payment Validated: Save?'); ?>"))
      {
       return true;
      }
     else
    ...
    $.ajax({
        ...
        beforeSend: function(xhr){
            ...
            if( $('#pin').val() == "" || $('#ccname').val() == "" || $('#ccyear').val() == "" || $('#ccmonth').val() == ""){
                 alert("<?php echo xla('Invalid Credit Card Values: Please correct'); ?>");
                 return false;
            }
            if( validate() != true){
                flag = 1;
                alert("<?php echo xla('Validation error: Fix and resubmit. This popup info is preserved!'); ?>");
                return false;
            }
...
$("#payfrm").on('submit', function(e){
    ...
    $.ajax({
        ...
        beforeSend: function(xhr){
            if( validate() != true){
                flag = 1;
                alert("<?php echo xla('Validation error: Fix and resubmit. Payment values are preserved!'); ?>");
                return false;
            }
        },
        error: function(xhr, textStatus, error){
            alert("<?php echo xla('There is a Post error'); ?>");
