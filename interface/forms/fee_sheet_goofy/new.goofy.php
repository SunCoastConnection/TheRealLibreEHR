...
//if(isset($_POST['bn_scrub'])){ /* Chi Wah 2017-10-13 added. */
//    require_once('../../../modules/scrubber/Scrubber.php');
//    $scrubber = new Scrubber($pid, $encounter, $GLOBALS['dbh']);
//    $scrubber->scrub();
//exit('Scrubbed');
//}
...
//  $icd10Code_invalidBecause              =NULL  /* Chi Wah added on 2017-10-16. */)
{
  ...
  echo "</td>\n";
  if ($codetype != 'COPAY') {
      if($cptCode_invalid_is){
          echo "  <td class='billcell cpt-code__invalid'>$strike1"   . text($code) . "$strike2</td>\n";
      }else if($icd10Code_invalid_is){
          echo "  <td class='billcell icd10-code__invalid'>$strike1" . text($code) . "$strike2</td>\n";
      }else{
  ...
  echo "  <td class='billcell'>$strike1" . text($code_text) . "$strike2</td>\n";
    if($cptCode_invalid_is){
        echo '<td class="billcell cpt-code__invalid">'  .htmlspecialchars($cptCode_invalidBecauseOfAnotherCptCode).'</td>'."\n";
    }else if($icd10Code_invalid_is){
        echo '<td class="billcell icd10-code__invalid">'/*.htmlspecialchars($icd10Code_invalidBecause)*/.'</td>'."\n";
    }else{
        echo '<td class="billcell"></td>'."\n";
    }
  echo " </tr>\n";
...
//$visit_row = sqlQuery("SELECT fe.date, fe.scrub_status, opc.pc_catname " .
...
.billcell.cpt-code__invalid,
.billcell.icd10-code__invalid{
    color: red;
}
</style>
...
<?php } ?>
  ...
  <td class='billcell'><b><?php echo xlt('Because of');?></b></td>
 </tr>
...
//      $iter['cpt_code_invalid_is'],                          /* Chi Wah added on 2017-10-13. */
//      $iter['cpt_code_invalid_because_of_another_cpt_code'], /* Chi Wah added on 2017-10-13. */
//      $iter['icd10_code_invalid_is'],                        /* Chi Wah added on 2017-10-16. */
//      $iter['icd10_code_invalid_because']                    /* Chi Wah added on 2017-10-16. */);
  }
}
...
<?php if (!$isBilled) { ?>
<input type='submit' class="cp-misc" name='bn_scrub' value='<?php
//echo xla(
    //($visit_row['scrub_status'] == 1
    //    ? 'Scrub' /* Chi Wah note: I think it is a good idea not to give user any hint, we want them to always scrub if they want to see the most up-to-date result. */
    //    : 'Scrub')
);?>' />
