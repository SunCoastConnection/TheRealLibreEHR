...
{
    //$last_report_id = sqlQuery("SELECT report_id as last_report_id FROM report_itemized WHERE numerator_label=? AND pass=? AND pid=? ORDER BY report_id DESC LIMIT 1", array("Numerator", 0, $patient_id));
    sqlStatement("UPDATE report_itemized SET pass=4 WHERE pid=? AND numerator_label=? ORDER BY report_id DESC LIMIT 1", array($patient_id, "Numerator"));
}
