...
<?php
 ...
 //////////// Implement NotMet
           if(isset($row['itemized_test_id']) && $row['pass_notmet'] > 0) {
            ...
            //error_log(print_r($row['pass_notmet'], true));
?>
...
//////////////////Implement Unreported
            ...
            //
            // if(isset($row['is_main'])) {
            //     $unreported_items = $failed_items - $row['pass_notmet'];
            // }

            if(isset($row['itemized_test_id']) && $row['unreported'] > 0) {
                $query = http_build_query(array(
                    'from_page' => 'pqrs_report',
                    'pass_id' => 'unreported',
                    'report_id' => attr($report_id),
                ...
                //error_log("if Unreported: ".print_r($row['unreported'], true));
?>
                <td style="text-align:center"><a href='patient_select.php?<?php echo $query; ?>' onclick='top.restoreSession()'><?php echo $row['unreported']; ?></a></td>
<?php
            } else {
                //error_log("else Unreported: ".print_r($row['unreported'], true));
?>
                <td style="text-align:center"><?php echo $row['unreported']; ?></td>
<?php
            }
?>
