...
class PQRSResult implements RsResultIF
{
    ...
    public $patientsUnreported; // Number of patients who we weren't reported. (No fail nor pass billing.code)
    ...
    public function format()
    {
        ...
        $rowFormat = array(
            ...
            'unreported' => $this->patientsUnreported['Unreported'],
            'percentage' => $this->percentage );
            $rowFormat = array_merge( $rowFormat, $this->rule );

            error_log(print_r($this->patientsNotMet['NotMet'], true) . " AND " . print_r($this->patientsUnreported['Unreported'], true));

        // If itemization is turned on, then record the itemized_test_id
        if ($GLOBALS['report_itemizing_temp_flag_and_id']) {
            $rowFormat = array_merge( $rowFormat, $this->itemized_test_id );
        }
error_log(print_r($rowFormat, true));
        return $rowFormat;
    }
}
